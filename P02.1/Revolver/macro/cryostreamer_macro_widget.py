...
class CryostreamerMacro(layout_temperature_macro.Ui_Form, default_macro.MacroControls):
    
    def __init__(self, parent=None):
        super(CryostreamerMacro, self).__init__(self)
    ...
    def action_show_settings(self):
        ...
        settings.toggle_elements_display(False,[settings.hotblower_label, settings.hotblower_settings])
    ...
    def action_open_dialog_add_macro(self):
        dialogs.AddCryostreamerMacroDialog(self).exec_()
    ...
    def action_start_logging(self, *args, **kwargs):
        ...
        if self.logWidget:
            ...
            graphOptions = {"title":"Cryostreamer temperature log", "xlabel":"Time [s]", "ylabel":"Temperature [K]"}
    ...
    def execute_macro(self, takeDark=None):
        ...
        """
        self.generate_macro_steps()
        device = devices.Cryostreamer(self.steps[0].devicePath)
        macro.STOP = False
        try:
    ...
    def action_macro_edited(self, item):
        ...
        try:
            if(coll == 3): 
                ...
                device = devices.Cryostreamer(devicePath)
