...
version 0.7
0.7 improvement - added abitily to switch the marker on/off
0.6 improvement - debugging for setting the correct marker size after startup
...
class MarkerCanvas(QWidget):
    ...
    def getSetOpacity(self, value = None):
        if(value!=None):
            ...
            elif(t is list or  t is tuple): # for a case when I forget to transorm some values
                value = float(value[0])
            if(t is float and value>=OPAMINIMUM and value<=OPAMAXIMUM+OPASTEP/2):
                self.opacity = value
...
class MarkerControl(QMainWindow):
    ...
    def initSelf(self):
        ...
        # create different tabs, each tab takes care for assigning its children events
    ...
    def createMarkerTab(self, tab):
        ...
        self.cbmarkervis = QCheckBox("marker visibility")
        self.cbmarkervis.setChecked(True)
        tgrid.addWidget(self.cbmarkervis, 5, 1, 1, 1, Qt.AlignLeft)
        
        tgrid.setRowStretch(6, 50)
        ...
        self.connect(self.cbmarkervis, SIGNAL("toggled(bool)"), self.updateMarkerVisibility)
    ...
    def readSettings(self):
        ...
        # position - should be set before we change shape, shape will take additional care for the round and square dimensions
            # position
        ...
        self.marker.getSetShape(value)
            ...
            # change size after we set the shape
        ...
        if(PYQT_VERSION < 0x40a00):
            ...
            value = self.settings.value(SETMARMCPOS, self.marker.moveCursorGetSetPos())
        ...
        if(PYQT_VERSION < 0x40a00):
            ...
            value = self.settings.value(SETMARMCPOS,self.marker.moveCursorGetSetVisibility())
        self.marker.moveCursorGetSetVisibility(value)
            # frame visibility
        if(PYQT_VERSION < 0x40a00):
            value = self.settings.value(SETDISPLAY, self.marker.getSetVisibility()).toBool()
        else:
            value = self.settings.value(SETDISPLAY, self.marker.getSetVisibility())
        ...
        if(PYQT_VERSION < 0x40a00):
            ...
            value = self.settings.value(SETMOVESTEPSMALL, self.marker.getSetMoveStepSmall())
        self.marker.getSetMoveStepSmall(value)
            # marker resize step
        if(PYQT_VERSION < 0x40a00):
            value =  self.settings.value(SETRESIZESTEP, self.marker.getSetResizeStep()).toInt()
        else:
            value = self.settings.value(SETRESIZESTEP, self.marker.getSetResizeStep())
        ...
        if(PYQT_VERSION < 0x40a00):
            ...
            value = self.settings.value(SETRESIZESTEPSMALL, self.marker.getSetResizeStepSmall())
        ...
        if(PYQT_VERSION < 0x40a00):
            ...
            value = self.settings.value(SETSCRDISTANCE, self.getSetScreenDistance())
        self.getSetScreenDistance(value)
            # object  distance
        if(PYQT_VERSION < 0x40a00):
            value =  self.settings.value(SETOBJDISTANCE, self.getSetObjectDistance()).toFloat()
        else:
            value = self.settings.value(SETOBJDISTANCE, self.getSetObjectDistance())
    ...
    def updateMarkerVisibility(self, bflag):
        if(bflag):
            self.marker.show()
        else:
            self.marker.hide()
    ...
    def updateCalibrationTab(self):
        ...
        try:
            (self.scrdistance, self.objdistance) = (float(str(self.ledistscreen.text())), float(str(self.ledistobj.text())))
        except ValueError:
    ...
    def createCMBFrameShapes(self):
        ...
        if(self.cmbframeshape.count()==0):
            for shape in frameshapes:
                ...
                for v in shape:
                    temp.append(v)
                self.cmbframeshape.addItem(strshape, temp)
        return
    ...
    def createCMBFrameColors(self):
        ...
        if(self.cmbframecolor.count()==0):
            for color in framecolors:
                ...
                pixmap.fill(color)
                icon = QIcon(pixmap)
                self.cmbframecolor.addItem(icon, "", color)
        return
    ...
    def createCMBMovableCursorColors(self):
        ...
        if(self.cmbcursorcolor.count()==0):
            for color in cursorcolors:
                ...
                pixmap.fill(color)
                icon = QIcon(pixmap)
                self.cmbcursorcolor.addItem(icon, "", color)
        return
    ...
    def getSetScreenDistance(self, value = None):
        if(value!=None):
            ...
            if(t  is QVariant): # 
                ...
                value = float(value[0])
            self.scrdistance = float(str(value))
    ...
    def getSetObjectDistance(self, value = None):
        if(value!=None):
            ...
            if(t  is QVariant): # 
                ...
                value = float(value[0])
            self.objdistance = float(str(value))
...
Step 2: Drag the calibration line accross the object with known size.
...
Reset calibration and clean the drawn line by pressing the right mouse button."""
    ...
    # process key events - Excape closes window and resets calibration, Enter - confirms the choice
    def keyPressEvent(self, event):
        k = event.key()
        m = event.modifiers()
    ...
    def closeEvent(self, event):
        if(self._bcloseready):
            ...
            if(self._firstpos):
                (x, y) = ((self._firstpos-self._secondpos).x(), (self._firstpos-self._secondpos).x())
                (x, y) = (float(x), float(y))
