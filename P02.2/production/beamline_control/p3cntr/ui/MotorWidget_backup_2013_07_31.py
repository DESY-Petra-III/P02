...
##            col+=1
            ...
            #func_callback = functools.partial(self.text_edited, )
            #self.connect(self.self.motordev_widget[-1][-1], QtCore.SIGNAL("textEdited(const QString&)"), )
...
##            col+=1         
            ...
            if(wdgtdevice.text().indexOf("zoom")>-1):
                tlist = ["step", 1, 10, 100]
            if(wdgtdevice.text().indexOf("Omega")>-1):
                tlist = ["step", 1, 2, 3, 5, 10, 15, 20, 30]
            if(wdgtdevice.text().indexOf("detector X")>-1):
                tlist = ["step", 10, 25, 50, 100]
            if(wdgtdevice.text().indexOf("Ruby Y")>-1):
                tlist.append(10.)
                tlist.append(20.)
                tlist.append(30.)
                tlist.append(40.)
