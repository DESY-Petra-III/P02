...
class CurrentSmallForm(SmallForm2.SmallForm2):
    def __init__(self,device_list,variable_list,parent=None,interval=1000,label_list=None,value_dicts=None,
        print_device_labels=False,ics_url=None,BS_list=None,BS_attrib_list=None):
...
##    print self.current,self.TopUp,self.TopUp_str,self.FOFB,self.FOFB_str,deviation
    ...
    for BS_index in range(len(self.BS_list)):
        if self.hutch_modification[BS_index]=="Enable beam operation":
            ...
            update_buttons()
            fetch_url(self.ics_url+"action=closebs&shutter=06_"+str(BS_index))
            while True:     
...
##                PyTango.DeviceProxy("P02/ics/1").BreakInterlock(BS_index+1)
    ...
    ##        print "before ICS-call",self.ics_url+"action=openbs&shutter=06_2"
            fetch_url(self.ics_url+"action=openbs&shutter=06_"+str(BS_index))
...
mono="P02/dcmener/mono.01"

App = QtGui.QApplication(sys.argv)
App.setFont(QtGui.QFont("Helvetica",16))

#form=CurrentSmallForm([PETRAIII,VacuumInterlock,BS1,BS2,PETRAIII_UndBPos],
form=CurrentSmallForm([PETRAIII,
                      undulator,mono,VacuumInterlock,BS0,BS1,BS2],
               [["BeamCurrent","DeclaredState","MessageText","FastOrbitFBStatusText","TopUpStatusText"],
	       ["Gap","Position"],
      	       ["Position"],
               ["FRONTEND_OFFEN"],
               ["BS0OffenDisplayState"],
               ["BS1OffenDisplayState"],
               ["BS2OffenDisplayState"],
#                ["Xposition","XpositionSoll",
#                 "Yposition","YpositionSoll",
#                 "Xangle","XangleSoll",
#                 "Yangle","YangleSoll"],
##                ["Position_x","Position_y","PosAndAvgCurr"],
##                ["PosAndAvgCurr"],
                 ],
                label_list=[["beam current [mA]","PETRA state","BKR message","fast orbit feedback","top-up mode"],
                ["gap [mm]","gap [eV]"],
                ["mono energy [eV]"],
                ["frontend"],
                ["beamshutter 0"],
...
#                ["X position","X target",
#                 "Y position","Y target", 
#                 "X angle","X ang target",
#                 "Y angle","Y ang target"],
##                ["QBPM x","QBPM y","Pos+AvgCurr"],
##              ["Pos+AvgCurr"],
                 ],
                value_dicts=[[None,None,None,{"Ein": "on","Aus":"off",None:"undefined","Unbestimmt":"undefined"},None],
                [None,None],[None],
                [{0:"closed",1:"open"}],
                #[{1:"closed",0:"open"}],
                #[{1:"closed",0:"open"}],
...
##                [None],
                ...
                ics_url="http://ics/tineinterface.php?",
                #BS_list=[["hasylab/piconditions/bs06_0_offen","hasylab/piconditions/bs06_0_geschlossen"],
                         #["hasylab/piconditions/bs06_1_offen","hasylab/piconditions/bs06_1_geschlossen"],
                         #["hasylab/piconditions/bs06_2_offen","hasylab/piconditions/bs06_2_geschlossen"]],
                #BS_list=[["P02/ics/1","P02/ics/1"],
                         #["P02/ics/1","P02/ics/1"],
                         #["P02/ics/1","P02/ics/1"]],
                BS_list=[["P02/shutter/1","P02/shutter/1"],
                         ["P02/shutter/1","P02/shutter/1"],
                         ["P02/shutter/1","P02/shutter/1"]],
                BS_attrib_list=["BS0OffenDisplayState","BS1OffenDisplayState","BS2OffenDisplayState"]
                )


if os.name=='posix':
    path="/data/P02user/Log_ShutterControl/"
elif os.name=='nt':
